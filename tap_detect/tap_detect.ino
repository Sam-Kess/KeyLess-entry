#define TOUCH_PIN T3 // GPIO 2 (T2)

// void setup() {
//   Serial.begin(115200);
//   delay(1000);
//   Serial.println("Simple touch read test");
// }

// void loop() {
//   int value = touchRead(TOUCH_PIN);
//   Serial.printf("Touch reading: %d\n", value);
//   delay(100);
// }
...
#define TOUCH_BUFFER_WIDTH
int touch_buff_idx = 0;
touch_value_t average_touch_buff[TOUCH_BUFFER_WIDTH] = {0};
...
bool isButtonPressed () {
    // if (touch_buff_idx++ == TOUCH_BUFFER_WIDTH) touch_buff_idx = 0;

    touch_value_t val = touchRead(4);

    if (val < TAP_THRESHOD) return true;
    return false;

    // touch_value_t last_avg = 0;
    // for (int i = 0; i < TOUCH_BUFFER_WIDTH ; i++) last_avg += average_touch_buff[i];
    // last_avg = last_avg / TOUCH_BUFFER_WIDTH;
